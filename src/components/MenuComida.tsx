    ...
    const readData = async () => {
      try {
        ...
        if (OrderSort.length === undefined) {
          ...
          await fetch(`${process.env.REACT_APP_IP_API}/api/products/menu`, {
            method: "GET",
            headers: myHeaders,
            redirect: "follow",
          })
          ...
          return (
            ...
            >
              ...
              <div className="container-fluid  d-flex flex-wrap justify-content-center ">
                {group.map((infoImg: any, index: number) =>
                  index <= 3 ? (
                    <Card
                      key={infoImg.id}
                      compras={compras}
                      ...
                      infoImg={infoImg}
                    ></Card>
                  ) : !openModal[openModal.indexOf(group[0].tipo) + 1] &&
                    index > 3 &&
                    4 >= index ? (
                    <ModalCollapseCard
                      key={Math.random()}
